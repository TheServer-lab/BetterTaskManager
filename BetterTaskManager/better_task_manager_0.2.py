# better_task_manager_with_tree.py  (extended)
"""
Better Task Manager — Tree + Per-process graphs + GPU/IO/Connections
- Per-process: CPU %, Memory %, GPU memory (MB if available), Disk I/O KB/s (read/write), Active network connection count
- GPU: tries pynvml first (NVIDIA), then nvidia-smi command as fallback. If not available or other vendor, GPU per-process shows N/A/0.
- Uses psutil + PyQt5 + pyqtgraph
"""
...
# Sampling / history
REFRESH_INTERVAL_MS = 1000   # sampling interval (ms)
HISTORY_LENGTH = 120         # number of samples kept (~2 minutes at 1s)

# Try pynvml (NVIDIA) first
GPU_BACKEND = None
try:
    import pynvml
    pynvml.nvmlInit()
    ...
    # will try nvidia-smi fallback later
    GPU_BACKEND = None

# Helper: try nvidia-smi fallback if pynvml missing
def query_nvidia_smi_pid_usedmem():
    """
    Try to run `nvidia-smi` to query per-process used GPU memory.
    Returns dict {pid: mem_mb}.
    Requires nvidia-smi present and accessible from PATH.
    """
    out = {}
    try:
        # Query compute/graphics processes with used memory (noheader, nounits)
        ...
        if sp.returncode != 0 or not sp.stdout.strip():
            # Try graphics processes query (older drivers)
            cmd2 = ["nvidia-smi", "--query-gpu=memory.used", "--format=csv,noheader,nounits"]
            # can't get per-process from this; return empty
            return {}
...
def get_gpu_process_memory_map():
    """
    Return mapping pid -> GPU memory used (MB). Tries multiple backends:
    - pynvml (preferred)
    - nvidia-smi (fallback)
    - otherwise returns {}
    """
    ...
    if GPU_BACKEND == "pynvml":
        try:
            ...
            for i in range(device_count):
                try:
                    ...
                    # Try graphics processes (some versions expose this)
                    try:
                    ...
                    for pinfo in plist:
                        ...
                        # many wrappers provide usedGpuMemory in bytes or in MiB depending on version
                        mem = getattr(pinfo, "usedGpuMemory", None) or getattr(pinfo, "used_memory", None) or getattr(pinfo, "memoryUsed", None)
                        if pid is None or mem is None:
                            continue
                        try:
                            val = float(mem)
                            # if value looks like bytes, convert to MB
                            if val > 1024 * 1024 * 2:
                                val = val / (1024 * 1024)
                        except Exception:
                            val = 0.0
        ...
        # try nvidia-smi command fallback
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Better Task Manager — Tree + Per-Process Graphs (GPU/IO/Net)")
        ...
        # per-process graphs: CPU, Mem, GPU mem, IO (r/w), Conns
    ...
    def _sample_processes(self):
        """
        Sample CPU, memory, io, connections for each process and maintain per-pid history.
        """
        current_pids = set()
        # GPU mapping (try backends)
        gpu_map = get_gpu_process_memory_map()

        # We'll iterate all processes (costly but typical)
        now = time.time()
        for p in psutil.process_iter(['pid','name']):
            ...
            # compute KB/s delta for IO using _last_proc_io
    ...
    def _rebuild_tree(self):
        ...
        # helper to create QTreeWidgetItem recursively with cycle protection
        self.tree.clear()
        visited = set()
        def make_item(pid):
        ...
        def add_children_safe(parent_item, pid):
            ...
            for child_pid in sorted(children_map.get(pid, [])):
                try:
                    child_info = proc_info.get(child_pid)
                    if not child_info:
                        continue
        ...
        # roots: pids whose parent not in proc_info (or ppid == 0/None)
    ...
    def fill_process_details(self, pid):
        ...
        # compute instantaneous IO KB/s and GPU mem from last sampled histories if present
        read_kbs = None
        write_kbs = None
        gpu_mb = None
        conn_count = None
        ...
        if gpu_mb is None or gpu_mb == 0.0:
            # if GPU backend exists but this pid not in map, show 0; otherwise show N/A
            if GPU_BACKEND or (shutil_which("nvidia-smi")):
...
# Utility helper used in details
...
# -------------------------
def main():
    # High-DPI attribute must be set before QApplication created
    try:
