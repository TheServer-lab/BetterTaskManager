...
"""
Better Task Manager — System Overview + Processes + Automation
- Process tree + table (keeps selection across refreshes)
...
- Adds "Status" column: Active, Dormant, Not Responding (best-effort)
- Automation tab:
    - Auto-kill unresponsive apps (timeout)
    - Global hotkey to kill either selected app, foreground app, or both (mode C)
- Persistent settings stored in automation_settings.json
- Optional extras: pynvml (NVML), pywin32 (win32gui/win32process), keyboard (global hotkeys)
Dependencies:
...
def pid_has_hung_window(pid):
    """
    Returns True if any top-level window owned by PID reports IsHungAppWindow(hwnd).
    Requires pywin32 (win32gui). If not available, return None.
    """
    if not WIN32_AVAILABLE:
        return None
    hung_found = False
    def enum_cb(hwnd, lparam):
        ...
        try:
            ...
            # check visible top-level windows only
...
class ProcessTableModel(QtCore.QAbstractTableModel):
    ...
    def update_snapshot(self, filter_text="", status_map=None):
        ...
        for p in safe_proc_iter(['pid','name','username','status','cmdline']):
            ...
            pid = info.get('pid')
            # attach status string if available
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if role == QtCore.Qt.BackgroundRole:
            # color row when Not Responding or Dormant
...
def is_safe_to_kill(pid):
    try:
        ...
        if name in PROTECTED_NAMES:
            return False
        # disallow killing SYSTEM-owned processes
        try:
            uname = p.username() or ""
...
class HotkeyThread(threading.Thread):
                ...
                if settings.get("hotkey_enabled"):
                    ...
                    if not self.registered:
                        try:
                            ...
                            self.registered = False
                    # sleep small and continue loop to allow dynamic changes
                else:
                    if self.registered:
                        try:
    ...
    def on_hotkey(self):
        try:
            ...
            # A: kill selected
            # B: kill foreground
            # C: both (prefer selected, fallback to foreground)
            if mode == "A":
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    # -------------------------
    # Save automation settings from UI
    # -------------------------
    def save_automation_settings(self):
        ...
        self.lbl_auto_status.setText("Saved")
    ...
    def refresh_all(self, force=False):
        ...
        # sample processes and system metrics (status computation inside)
        self._sample_processes()

        # update model snapshot with latest proc_status map
    ...
    def _sample_processes(self):
        ...
        # per-process GPU mapping
        gpu_map = get_gpu_process_memory_map()

        current_pids = set()
        # temporary new status map
        new_status = {}

        # get foreground pid for active detection
        ...
        for p in safe_proc_iter(['pid','name']):
            ...
            if active:
                ...
                # reset first_hung if recovered
                self._first_hung_detected_time.pop(pid, None)

            # Hung / Not Responding detection
            ...
            if WIN32_AVAILABLE:
                ...
                # fallback: very long inactivity + zero cpu and zero IO
            ...
            # set status string based on logic
            status_str = "Active"
            # compute dormant threshold
            ...
            if not_responding:
                ...
                # record first detection time if not already
            ...
            # Auto-kill logic: if Not Responding and enabled and timed out long enough
            try:
...
# -------------------------
def main():
    # High DPI attributes before QApplication
    try:
